/**
 * Cursor CLI adapter.
 * Spawns the `cursor-agent` CLI in headless mode and streams output.
 */
...
export class CursorCLIAdapter extends BaseAdapter {
	...
	private rateLimitDetector: CursorRateLimitDetector;

	constructor() {
		super();
		this.rateLimitDetector = new CursorRateLimitDetector();
	}
	...
	async *execute(task: AgentTask): AsyncIterable<AgentEvent> {
		...
		try {
			...
			if (debugMode) {
				...
				console.error(
					`[DEBUG] Executing: cursor-agent ${args.map((a) => (a.includes(' ') ? `"${a}"` : a)).join(' ')}`,
				);
			}

			// Spawn Cursor CLI with piped stdio
	...
	getContextFile(): string {
		return this.config?.provider.contextFile ?? '.cursorrules';
	}
	...
	private buildArgs(task: AgentTask, _sessionId: string): string[] {
		...
		// Max steps
		const maxSteps = (task.options as CursorCLIOptions | undefined)?.maxSteps;
		if (maxSteps) {
			args.push('--max-steps', String(maxSteps));
		}

		// Resume support (Cursor uses --resume with session ID)
		if (task.sessionId) {
		...
		if (existsSync(contextPath)) {
			// Cursor automatically reads .cursorrules from the working directory
		}
...
/**
 * Rate limit detector for Cursor CLI
 */
class CursorRateLimitDetector implements RateLimitDetector {
 ...
 */
export function createCursorCLIAdapter(): CursorCLIAdapter {
